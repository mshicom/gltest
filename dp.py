    ...
    code = r"""
        ...
        for (size_t m=1,md=0; m<=M; m++,md++)
            for(size_t n=1,nd=0; n<=N; n++,nd++ )
            {
                ...
                float Edata = err*err;
                /* The state S[l,r] means there are l pixels in the
...
def interceptLine(xmin,xmax,ymin,ymax, x0, y0, dxy):
    # xmin,xmax,ymin,ymax, x0, y0, dxy = 1,w-1,1,h-1, x, y, dxy_l
...
fac_mat = np.diag([factor, factor, 1])
K = fac_mat.dot(K0)
