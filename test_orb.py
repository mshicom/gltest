...
#%%
    ...
    d,vm,var = f0.searchEPL(fs[9], K, dmin=iD(5), dmax=iD(0.1), win_width=4) #
    ...
    def dumpToFs():
        ...
        for i in range(1,len(fs)):
            i_str = str(i)
            ec = EpilineCalculator(pref[0], pref[1], getG(f0,fs[i]), K)
            vmin, vmax, d_min, d_max, valid_mask = ec.getLimits(f0.im.shape)
